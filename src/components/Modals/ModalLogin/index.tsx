...
export default function ModalLogin({ isOpen, onClose }: ModalLoginProps) {
  ...
  const handleContinueWithTemporaryPassword = async (userData: any) => {
    ...
    const handleCreateUser = async () => {
      ...
      const response = await userService.createPatientByUser(userData, "");
  ...
  const handleLogin = async () => {
    ...
    try {
      if (
        emailLogin === EnvConfig.ADMIN_EMAIL &&
        passwordLogin === EnvConfig.ADMIN_PASSWORD
      ) {
        const loginData = {
          grant_type: "password",
          username: emailLogin,
        ...
        try {
          ...
          if (response.status === 200) {
            ...
            try {
              console.log(
                "Fazendo chamada para /api/users/me com token do admin:",
                response.data.access_token
              );
              const userMeResponse = await userService.getCurrentUser(
                response.data.access_token
              );

              console.log(
                "Resposta do /api/users/me para admin:",
              ...
              if (userMeResponse.status === 200) {
                ...
                console.log(
                  "is_first_access do admin:",
                  userMeData.is_first_access
                );
                const adminData = {
                  ...response.data,
                  ...userMeData,
                ...
                if (adminData.is_first_access === true) {
                  console.log(
                    "Primeiro acesso do admin detectado - exibindo campos de senha"
                  );
      ...
      if (response.status === 200) {
        ...
        try {
          console.log(
            "Fazendo chamada para /api/users/me com token:",
            response.data.access_token
          );
          const userMeResponse = await userService.getCurrentUser(
            response.data.access_token
          ...
          if (userMeResponse.status === 200) {
            ...
            console.log(
              "Verificando is_first_access:",
              userData.is_first_access,
              typeof userData.is_first_access
            );
            if (userData.is_first_access === true) {
              console.log(
                "Primeiro acesso detectado - exibindo campos de senha"
              );
              setShowPasswordFields(true);
              showToast(
                "Bem-vindo!",
          ...
          <div className="px-6 pb-6">
                    ...
                    <>
                      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mt-4">
                        ...
                        <p className="text-sm text-blue-700 mb-4">
                          Defina uma nova senha para sua conta ou continue com a
                          senha temporária.
                        </p>
                        ...
                        <div className="gap-2 mt-4">
                          <ButtonPrimary
                            onClick={() =>
                              handleChangePassword(currentUserData)
                            }
                            disabled={isLoading}
                          >
                          ...
                          <ButtonPrimary
                            onClick={() =>
                              handleContinueWithTemporaryPassword(
                                currentUserData
                              )
                            }
                            disabled={isLoading}
            ...
            {activeTab === "register" && (
              <p className="text-xs text-neutral-19 mt-4 text-center">
                ...
                <a href="#" className="text-neutral-20 hover:underline">
                  termos de uso
                </a>{" "}
                e{" "}
                <a href="#" className="text-neutral-20 hover:underline">
